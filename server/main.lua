...
RegisterServerEvent('inventory:server:GiveItem', function(target, name, amount, slot)
	...
	if not item then
		QBCore.Functions.Notify(src, Lang:t('notify.infound')); return
	end
	if item.name ~= name then
		QBCore.Functions.Notify(src, Lang:t('notify.iifound')); return
	end
...
QBCore.Commands.Add('giveitem', 'Give An Item (Admin Only)', {
    ...
    if Player then
        if itemData then
            ...
            -- RC car vehicle spawn logic
            if itemData['name'] == 'rc_monster' or itemData['name'] == 'rc_ruiner' or itemData['name'] == 'rc_sultanrs' or itemData['name'] == 'rc_bandito' or itemData['name'] == 'rc_bandito_offroad' or itemData['name'] == 'rc_bandito_trophy' or itemData['name'] == 'rc_bandito_gang' or itemData['name'] == 'rc_bandito_big' or itemData['name'] == 'rc_bandito_midnight' then
                exports['qbcorestore-rccar']:CreateNewVehicle(id, itemData['name'], 1, false, nil, true)
                return
            end
